...
\begin{abstract}
C++, Java, C\#, Python, Ruby, JavaScript are the most powerful object-oriented programming languages, if language power is defined as the number of \emph{features} available to a programmer.
...
Here, \ff{p1} is an object that can be used as an argument of \ff{f}. It is a copy of an abstract object \ff{heap.pointer}, which expects two parameters: 1)~an absolute address in memory, and 2)~the size of the memory block it points to, in bytes. Its attribute \ff{block} expects two attributes: 1) the number of bytes in the heap, and 2)~an abstract object that can encapsulate \ff{bytes} which were just read from memory.
...
Here, the atom \ff{malloc} allocates a segment of bytes in the \ff{heap}. Later, the atom \ff{free} releases it. The attribute \ff{ret} is made constant in order to avoid its recalculation after it's ``returned'' (this mechanism is explained in \cref{sec:destructors} where destructors are discussed).
...
The restrictions enforced by the Java type system at compile time through types \ff{Cart} and \ff{Book} may be represented in \eolang{} by means of decorators, for example:
...
Here, the \ff{type} object implements all necessary restrictions the Java type system may provide for the type \ff{Cart} and its methods.
...
Here, the synthetic \ff{source} attribute represents the location in the source code file with the original C code. It's important to make sure during translation that the name \ff{source} doesn't conflict with a possibly similar name in the object.
