...
The more features a language has, the harder its programs are to reason
  ...
  mccabe1976complexity, basili1984software}.
...
malloc.empty > [] > app
  [c]
    ...
    seq * > @
      ...
      cnt.next
\end{ffsave}
\twoffinputs{0.4}{closure.js}{closure.eo}
...
\begin{ffsave}[encapsulation.java]
class Cart {
  private int rate = 10;
  ...
  int total(int p) {
    return p + tax(p); } }
\end{ffsave}
\begin{ffsave}[encapsulation.eo]
[] > cart
  p.plus > [p] > total
    p.div 10
\end{ffsave}
\twoffinputs{0.45}{encapsulation.java}{encapsulation.eo}
...
\begin{ffsave}[static-field.java]
class Book {
  static int count = 0;
  Book() {
    count += 1;
...
malloc.empty > [] > app
  [count]
    seq * > @
      ...
      count.as-number
\end{ffsave}
\twoffinputs{0.5}{static-field.java}{static-field.eo}
...
\begin{ffsave}[polymorphism.java]
class Animal {
  String sound() {
    return "?";
  }
...
speak dog
\end{ffsave}
\twoffinputs{0.5}{polymorphism.java}{polymorphism.eo}
...
\end{ffsave}
\begin{ffsave}[interface.eo]
[] > shape
  [] > area
[side] > square
  side.times side > [] > area
\end{ffsave}
\twoffinputs{0.5}{interface.java}{interface.eo}
